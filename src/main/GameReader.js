...
import { fetchOffsetLookup, fetchOffsets } from './offsetStore';
...
export default class GameReader {
    ...
    async loop() {
        ...
        if (this.PlayerStruct &&
            ...
            if ((this.gameCode || this.isLocalGame) && playerCount) {
                ...
                const gameOptionsPtr = this.readMemory('ptr', this.gameAssembly.modBaseAddr, this.offsets.gameoptionsData);
    ...
    async initializeoffsets() {
        ...
        if (this.offsets.newGameOptions) {
            const gameOptionsManager = this.findPattern(this.offsets.signatures.gameOptionsManager.sig, this.offsets.signatures.gameOptionsManager.patternOffset, this.offsets.signatures.gameOptionsManager.addressOffset);
    ...
    initializeWrites() {
        ...
        this.writeString(shellCodeAddr + 0xd5, `<size=85%><color=#BA68C8>BetterCrewLink v${appVersion}</color></size>\n<size=60%><color=#BA68C8>https://bettercrewlink.app</color></size><size=85%>\nPing: {0}ms</size>`);
